/*
  # Create locations table and related functions
  ...
  2. Security
    ...
    - Add audit logging for location changes
...
CREATE POLICY "Global admins can access all locations" 
  ...
  FOR ALL
  TO authenticated
  USING (is_global_admin(uid()))
  WITH CHECK (is_global_admin(uid()));
...
CREATE POLICY "Users can view company locations" 
  ...
  USING (
    EXISTS (
      ...
      AND user_companies.user_id = uid()
    )
  );
...
-- Create audit logging function for locations
