...
-- Create a function to log security changes safely
CREATE OR REPLACE FUNCTION log_security_change(
...
DECLARE
  v_user_id uuid;
BEGIN
  -- Get current user ID if available, otherwise don't log
  v_user_id := auth.uid();
...
-- Log the security change safely
SELECT log_security_change('Added protection for is_global_admin field');